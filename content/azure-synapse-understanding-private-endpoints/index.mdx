...
canonical: "https://www.thedataguy.blog/azure-synapse-understanding-private-endpoints/#time-to-put-it-all-together"
---
...
You might've noticed in the list of private endpoint, we only had 3 of them while your workspace has 4 of them.
...
[private endpoint connections](#private-endpoint-connections) and [private link hub](#private-link-hub), lets' take
...
1. You have 2 storage accounts, one for the workspace file system (this is required by Synapse and setup during the 
deployment) and the another to store any audits and logs.
...
3. Any spark pools create will create virtual machines behind the scenes. These will also
be hosted inside the managed virtual network (vNet).

4. The workspace studio UI is actually a web app. For clarity purpose, I've included this
as part of the Synapse Worskspace perimeter but it actually resides outside. The  following
diagram shows this:
...
5. The Serverless SQL pool  serverless, will not be physically deployed in the 
vNet but will will communicate back and forth via private endpoints. 

6. Same as the Serverless SQL pools, it will not be physically deployed in the 
vNet but will will communicate back and forth via private endpoints. 

<Callout>
  💡 Remember the 2 managed private endpoints created when you deployed your new Synpase
...
1. When you create a storage account that limits access to certain vNets, endpoints
are needed to allow the connection and authentication

3. Similar to how Synapse needs private endpoints to communicate with the 
storage account, any external systesm or people that need to read or write
...
need a [managed prive endpoint](#managed-private-endpoints) like we mentioned previously.
...
<Callout>
  ...
  enabled. Hence the reason why we have 2 storage account in the diagram.
</Callout>

### Private endpoints in customer owned vNet

As we mentioned previously on the private endpoints for the storage accounts in the
customer's vNet, the same needs to be done for the 3 Synapse endpoints:
...
the nuances of each private endpoints.

The last piece of the puzzle that can cause issues would be authentication and access control. 

I can't recommand strongly enough that you go through this documentation which outlines all the steps you need to take.
