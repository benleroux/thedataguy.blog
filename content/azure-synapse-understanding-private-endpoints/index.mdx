...
~~Google~~ Bing defines firewall as _"... a security device that monitors 
...
block  traffic to your workspace via the internet.
...
In short, you can use a private IP to connect to a public service... Well, kind of as you still need to
use a DNS, but more on that later.

Every workspace comes with a few endpoints which are used to connect to from various applications: 
...
through a **DNS** redirection to a private DNS.

<Callout>
  <b>D</b>omain <b>N</b>ame <b>S</b>ervice translates domain names to IP addresses so browsers can load Internet resources
</Callout>
...
3. Any spark pools will create virtual machines behind the scene. These will also
...
as part of the Synapse workspace perimeter but it actually resides outside. The  following
diagram shows this:
...
5. The Serverless SQL pool will not be physically deployed in the 
vNet but will communicate back and forth via private endpoints. 
...
need a [managed private endpoint](#managed-private-endpoints) like we mentioned previously.
